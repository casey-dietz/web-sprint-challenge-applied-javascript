...
1. What is the DOM? It is the document object model. It is a representation of a webpage that Javacript can use.
2. What is an event? Events are actions or occurences that happen in the system you are programming, which the system tells you about so you can respond to them in some way if desired.
3. What is an event listener? An event listener is a procedure in javscript that waits for an event to occur, such as a user clicking the mouse.
4. Why would we convert a NodeList into an Array? Because you can not perform array methods on a nodelist. In order to do that we use array.from to convert the nodelist into an array and now the array methods are availble to us.
